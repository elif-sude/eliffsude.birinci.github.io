...
// gameoverdansonra oyun devam ediyor. alert kaldırıdı
/*const wordEl = document.getElementById('word');
const popup = document.getElementById('popup-container');
const messageEl = document.getElementById('success-message');
const wrongLettersEl = document.getElementById('wrong-letters');
...
const stages = [
    {
        words: ["paradigm", "synopsis", "phenomenon", "algorithm", "hypothesis"],
        hint: "Scientific and Technical Terms",
    },
    {
        words: ["philosophy", "renaissance", "architecture", "literature", "astronomy"],
        hint: "Fields of Study",
    },
    {
        words: ["archipelago", "metamorphosis", "serendipity", "quintessential", "conundrum"],
        hint: "Rare and Difficult Words",
    },
];
...
window.addEventListener('keydown', e => {
    if (gameOver) return; // If the game is over, no key presses will be accepted
        ...
        setTimeout(() => {
            ...
            if (currentWordIndex === stages[currentStage - 1].words.length) {
                if (currentStage === stages.length) {
                    ...
                    showStagePopup(); // Show stage completed popup
                }
            } else {
                correctLetters = [];
                wrongLetters = [];
...
// Yeni oyun başladığında oyuncuya açıklama mesajı gösteren fonksiyon
...
function startStage() {
    if (currentStage === 1) {  // Sadece ilk etapta açıklama göster
        showGameExplanation(); // Oyuncuya açıklama göster
    }
    correctLetters = [];
    wrongLetters = [];
    currentWordIndex = 0;
...
// Başlangıçta oyunu başlatmadan önce açıklama göster
function startStage() {
    showGameExplanation(); // Oyuncuya açıklama göster
    correctLetters = [];
    wrongLetters = [];
